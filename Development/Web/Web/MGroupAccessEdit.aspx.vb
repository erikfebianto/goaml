...
Public Class MGroupAccessEdit
    ...
    Public Property GroupName() As String
        Get
            Return Session("MGroupAccessEdit.GroupName")
        End Get
        Set(ByVal value As String)
            Session("MGroupAccessEdit.GroupName") = value
        End Set
    End Property
    ...
    Public Property objMGroupMenuSettting() As List(Of NawaDAL.MGroupMenuSettting)
        Get
            Return Session("MGroupAccessEdit.objMGroupMenuSettting")
        End Get
        Set(ByVal value As List(Of NawaDAL.MGroupMenuSettting))
            Session("MGroupAccessEdit.objMGroupMenuSettting") = value
        End Set
    End Property
    ...
    Public Property ObjMGroupMenuSetttingEdit() As NawaDAL.MGroupMenuSettting
        Get
            Return Session("MGroupAccessEdit.ObjMGroupMenuSetttingEdit")
        End Get
        Set(ByVal value As NawaDAL.MGroupMenuSettting)
            Session("MGroupAccessEdit.ObjMGroupMenuSetttingEdit") = value
        End Set
    End Property

    Public Property ObjModule() As NawaDAL.Module
        Get
            Return Session("MGroupAccessEdit.ObjModule")
        End Get
        Set(ByVal value As NawaDAL.Module)
            Session("MGroupAccessEdit.ObjModule") = value
        End Set
    End Property
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Try
                ...
                GridpanelAdd.Title = ObjModule.ModuleLabel & " Edit"
    ...
    Sub LoadData()
        ...
        Dim intgroupid As Integer = NawaBLL.MGroupMenuBLL.GetGroupMenuIDByname(GroupName)
        objMGroupMenuSettting = NawaBLL.MGroupAccessBLL.GetMGroupMenuSettingNew(intgroupid)
    ...
    Protected Sub RemoteMove(sender As Object, e As RemoteMoveEventArgs)
        Try
            ...
            Do While Not objMGroupMenuSettting.Find(Function(x) x.mMenuID = newid) Is Nothing
                intcounter += 1
    ...
    Sub UpdateNOdeAnak(objParent As MGroupMenuSettting, strOldnode As String, objsubmitmode As Ext.Net.Node)
        Dim objOldnode As Node = FindNodeById(TreePanelMenu.Root, objParent.mMenuID)
        'objOldnode.NodeID = objParent.mMenuID

        For Each item As Node In objOldnode.Children
            Dim objchild As MGroupMenuSettting = objMGroupMenuSettting.Find(Function(x) x.mMenuParentID = strOldnode And x.mMenuID = item.NodeID)
            Dim stroldchildnode As String = item.NodeID
            ...
            'Dim objchildsubmitNode As Ext.Net.Node = objsubmitmode.Children.Contains(Function(x) x.NodeID = stroldchildnode)


            '            objsubmitmode.ToProxyNode.FirstChild.Set("mMenu", objchild)



            item.NodeID = item.ParentNode.NodeID & "/" & objchild.urutan
            item.CustomAttributes(0).Value = Newtonsoft.Json.JsonConvert.SerializeObject(objchild)

            objchild.mMenuID = item.NodeID
            objchild.mMenuParentID = item.ParentNode.NodeID
            Dim objsubmitproxy As Ext.Net.NodeProxy = TreePanelMenu.GetNodeById(stroldchildnode)

            objsubmitproxy.Set("mMenu", objchild)
            objsubmitproxy.Reload()



            UpdateNOdeAnak(objchild, stroldchildnode, objsubmitmode)
    ...
    Protected Sub Store_ReadData(sender As Object, e As StoreReadDataEventArgs)
        Try
            ...
            If Not e Is Nothing Then
                ...
                strfilter = NawaBLL.Nawa.BLL.NawaFramework.GetWhereClauseHeader(e)
            End If
        ...
        Try
            ...
            If NawaBLL.MGroupAccessBLL.IsDataValidEdit(GroupName, ObjModule) Then
                ...
                If NawaBLL.Common.SessionCurrentUser.FK_MRole_ID.ToString = 1 OrElse Not (ObjModule.IsUseApproval) Then
                    ...
                    NawaBLL.MGroupAccessBLL.SaveEditTanpaApproval(objData, ObjModule, "$AppPathApplication$")
                ...
                Else
                    NawaBLL.MGroupAccessBLL.SaveEditApproval(objData, ObjModule)
