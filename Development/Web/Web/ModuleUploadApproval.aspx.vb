...
Public Class ModuleUploadApproval
    Inherits Parent
    Public objFormModuleApproval As NawaBLL.FormModuleApproval
    Public Property strWhereClause() As String
        Get
            Return Session("ModuleUploadApproval.strWhereClause")
        End Get
        Set(ByVal value As String)
            Session("ModuleUploadApproval.strWhereClause") = value
        End Set
    End Property
    Public Property strOrder() As String
        Get
            Return Session("ModuleUploadApproval.strSort")
        End Get
        Set(ByVal value As String)
            Session("ModuleUploadApproval.strSort") = value
        End Set
    End Property
    Public Property indexStart() As String
        Get
            Return Session("ModuleUploadApproval.indexStart")
        End Get
        Set(ByVal value As String)
            Session("ModuleUploadApproval.indexStart") = value
        End Set
    End Property
    ...
    Protected Sub Store_ReadData(sender As Object, e As StoreReadDataEventArgs)
        Try
            ...
            If strWhereClause.Trim.Length = 0 Then
                strWhereClause += " modulename='" & strModulename & "' and createdby in ( SELECT m.UserID FROM MUser m WHERE m.FK_MRole_ID IN ( SELECT c.FK_MRole_ID FROM muser c WHERE c.UserID='" & NawaBLL.Common.SessionCurrentUser.UserID & "') AND m.UserID<>'" & NawaBLL.Common.SessionCurrentUser.UserID & "')"
            Else
                strWhereClause += " and  modulename='" & strModulename & "' and createdby in ( SELECT m.UserID FROM MUser m WHERE m.FK_MRole_ID IN ( SELECT c.FK_MRole_ID FROM muser c WHERE c.UserID='" & NawaBLL.Common.SessionCurrentUser.UserID & "') AND m.UserID<>'" & NawaBLL.Common.SessionCurrentUser.UserID & "')"
            End If
            Me.strOrder = strsort
