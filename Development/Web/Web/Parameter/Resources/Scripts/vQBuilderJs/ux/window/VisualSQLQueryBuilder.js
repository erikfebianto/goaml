...
// SQLTableSprite --> used to draw join between two table on    
...
// SQLTableModel & SQLTableStore --> Used to save information about used table
...
// SQLJoin & JoinStore --> Used to save information about join between two table
...
// SQLFieldsModel & SQLFieldsStore --> Used to save information about used field (exclude join field)
...
// SQLSelect --> digunakan untuk membangun query dari field- yang terselect
Ext.define('Ext.ux.window.visualsqlquerybuilder.SQLSelect', {
    ...
    toString: function() {
        ...
        this.fields.each(function(field) {
            ...
            // any criteria
            if (field.get('criteria') != '' && field.get('having')==false) {
            ...
            // any having criteria
            if (field.get('criteria') != '' && field.get('having')==true) {
                aHavingFields.push(field);
            }
            // check for sorting
            if (field.get('sortType')) {

                if (field.get('sortOrder')) {
                    field.data.sortOrderText = ' desc';
                } else {
                    field.data.sortOrderText = '';
                }
                ...
                if ((k + 1) < aTables.length) {
                    ...
                    if (aJoin.length > 0) {
                        ...
                        if (leftTable.get('tableAlias') != '') {
                            ...
                            joinSQL = joinSQL + leftTable.get('tableName') + ' ' + leftTable.get('tableAlias') + '\n' + joinType + ' JOIN ';
                        } else {
                            //no alias
                            joinSQL = joinSQL + leftTable.get('tableName') + '\n' + joinType + ' JOIN ';
                        }
        ...
        for (var i = 0, l = aOutputFields.length; i < l; i++) {
            ...
            if (aOutputFields[i].get('alias') != '') {
                ...
                selectFieldsSQL = (i == 0 ? '\n ' : '') + selectFieldsSQL + aOutputFields[i].get('expression') + ' AS ' + aOutputFields[i].get('alias') + fieldSeperator;
            } else {
                // no field alias

                selectFieldsSQL = (i == 0 ? '\n ' : '') + selectFieldsSQL + aOutputFields[i].get('expression') + fieldSeperator;
        ...
        for (var i = 0, l = aCriteriaFields.length; i < l; i++) {
            if (aCriteriaFields[i].get('having') == false) {
                if (i == 0) {
        ...
        // is having criteria
        for (var i = 0, l = aHavingFields.length; i < l; i++) {
            if (aHavingFields[i].get('having') == true) {
                if (i == 0) {
                    havingSQL = havingSQL + '\nHAVING ';
                } else {
                    havingSQL = havingSQL + 'AND ';
                }
                if (i == aHavingFields.length - 1) {
                    fieldSeperator = '';
                } else {
                    fieldSeperator = '\n';
                }
                havingSQL = havingSQL + aHavingFields[i].get('expression') + ' ' + aHavingFields[i].get('criteria') + fieldSeperator;
            }
        }
        ...
        for (var i = 0, l = aOrderFields.length; i < l; i++) {
            ...
            orderBySQL = orderBySQL + aOrderFields[i].get('expression') + aOrderFields[i].get('sortOrderText') + fieldSeperator;
        }

        return sqlOutput + selectFieldsSQL + fromSQL + criteriaSQL + groupBySQL + havingSQL + orderBySQL;
    },
    ...
    addFieldRecord: function(record, bOutput, column) {
        ...
        if (column != undefined) {
            model.set('alias', column.alias);
            model.set('grouping', column.grouping);
            model.set('criteria', column.criteria);
            model.set('sortType', column.sortType);
            model.set('sortOrder', column.sortOrder);
            model.set('having', column.having);
            if (column.output != undefined) {
                model.set('output', column.output);
            }
            if (column.expression != undefined) {
                model.set('expression', column.expression);
            }
        }
        this.addField(model);
    },
    ...
    updateJoin: function(joinID, joinType) {
    
        var join;
        join = this.joins.getById(joinID);
        this.joins.remove(join);

        join.data.joinType = joinType;

        this.joins.add(join);

    },
    updateJoinCondition: function(joinID, joinCondition) {
    
        var join, joinType;
        join = this.joins.getById(joinID);
        joinType = join.data.joinType == undefined ? 'INNER' : join.data.joinType;

        this.joins.remove(join);

        join.data.joinCondition = joinCondition;
        join.data.joinType = joinType;
    ...
    getJoin: function(joinID) {
        return this.joins.getById(joinID);
    },
...
// SQLTablePanel --> Panel untuk meletakkan table-table yang digunakan dalam query
...
// SQLOutputPanel --> Panel untuk nemampilkan sql syntax
...
// SQLFieldsGrid --> Panel untuk menampilkan filed-field yang ter-select
Ext.define('Ext.ux.window.visualsqlquerybuilder.SQLFieldsGrid', {
    ...
    columns: [{
        ...
        items: [{
            icon: 'Resources/img/vQBuilderImg/up_arrow.gif',
            ...
            icon: 'Resources/img/vQBuilderImg/down_arrow.gif',
            getClass: function(value, metadata, record){
                var store, index;
                store = record.store;
                index = store.indexOf(record);
...
Ext.define('Ext.ux.window.visualsqlquerybuilder.SQLTableTree', {
    ...
    initComponent: function(){
        ...
        this.store = Ext.create('Ext.data.TreeStore', {
            ...
            proxy: {
                type: 'ajax',
                url: _rootUrl + '/QueryBuilderData.aspx?GetTableList=all'
            }
        });
...
Ext.define('Ext.ux.window.visualsqlquerybuilder.SQLTableGrid', {
    ...
    initComponent: function () {
        ...
        this.selModel = Ext.create('Ext.selection.CheckboxModel', {
            ...
            checkOnly: true,
            listeners: {
                select: function (selModel, data,index) {
                    
                    // add new rows to the SQLFieldsGrid after a selection change
                    var column;
                    
                    if (selectedField.length > 0) {
                        column = jQuery.grep(selectedField, function(e){ return e.field == data.raw.field; })[0]
                        if (column != undefined) {
                            if (column.hasSelect == true) {
                                column = undefined;
                            } else {
                                column.hasSelect = true;
                            }
                        }
                    }
                    
                    ux.vqbuilder.sqlSelect.addFieldRecord(data, true,column);
                },
...
// SQLTable --> window ini digunakan untuk menempelkan panel SQLTableGrid, sehingga panel SQLTableGrid bisa digeser-geser
Ext.define('Ext.ux.window.visualsqlquerybuilder.SQLTable', {
    ...
    listeners: {
        ...
        afterrender: function() {
            this.initAlias();
            this.initSelectedFiled();
            //            ,
            //            afterrender: function () {
            //window.selModel.select(1);
            //            }
        }
    },
    initSelectedFiled: function() {

        var x = this.items;
    },
    initAlias: function() {
        if (this.titleAlias != '') {
            var table, header, title, titleId, titleAlias;
            ...
            header.insert(0, [{
                    ...
                    initComponent: function() {
                        ...
                        if (titleAlias != this.parentCmp.origValue) {
                            this.parentTableModel.set('tableAlias', titleAlias);
                            this.parentCmp.origValue = titleAlias;
                        }
    ...
    showSQLTableCM: function(event, el) {
        ...
        cm = Ext.create('Ext.menu.Menu', {
            items: [{
                    ...
                    text: 'Remove Table',
                    icon: 'Resources/img/vQBuilderImg/delete.gif',
                    handler: Ext.Function.bind(function() {
                        // remove the sqltable
                        this.close();
                    }, this)
                    ...
                    text: 'Close Menu',
                    icon: _rootUrl + 'Resources/img/vQBuilderImg/cross.gif',
                    handler: Ext.emptyFn
                }]
        });
        // show the contextmenu next to current mouse position
    ...
    initComponent: function() {
        ...
        store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                url: _rootUrl + '/QueryBuilderData.aspx?GetTableList=getTableInfo',
                extraParams: {
...
// VisualSQLQueryBuilder --> window utama untuk menampilkan semua panel yang telah diinisialisasi sebelumnya
Ext.define('Ext.ux.window.VisualSQLQueryBuilder', {
    ...
    listeners: {
        beforeclose: function() {
//            jQuery('#vqbQueryhdn').next().remove();
//            jQuery('#vqbQueryhdn').after('<pre id="vqbQuery" style="min-height: 300px" class="brush:sql" name="vqbQuery">' + ux.vqbuilder.sqlSelect + ' </pre>');
    ...
    initComponent: function() {
        ...
        this.dockedItems = [{
            ...
            items: [{
                    ...
                    text: "Save",
                    xtype: 'splitbutton',
                    icon: _rootUrl + "Resources/img/vQBuilderImg/icon-save.gif",
                    listeners: {
                        click: function() {
                            jQuery('#ViewDesignerQuery').val(ux.vqbuilder.sqlSelect);
...
//                            SyntaxHighlighter.highlight();
                            ...
                            var tables = [];
                            var joins = [];
                            $.each(ux.vqbuilder.sqlSelect.fields.data.items, function(i, val) {
                                fields.push(val.data);
                            });
                            $.each(ux.vqbuilder.sqlSelect.tables.data.items, function(i, val) {
                                tables.push(val.data);
                            });
                            $.each(ux.vqbuilder.sqlSelect.joins.data.items, function(i, val) {
                                joins.push(val.data);
                            });

                            orgnObj = { fields: fields, tables: tables, joins: joins };
                            CloseWindow();
                            //ShowMessages('Query telah berhasil disusun');
                        }
                    }
                }]
        }];
