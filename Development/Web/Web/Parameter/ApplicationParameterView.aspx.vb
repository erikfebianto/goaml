...
Partial Class ApplicationParameterView
    Inherits Parent
    Public objFormModuleView As NawaBLL.FormModuleView
    Public Property strWhereClause() As String
        Get
            Return Session("ApplicationParameterView.strWhereClause")
        End Get
        Set(ByVal value As String)
            Session("ApplicationParameterView.strWhereClause") = value
        End Set
    End Property
    Public Property strOrder() As String
        Get
            Return Session("ApplicationParameterView.strSort")
        End Get
        Set(ByVal value As String)
            Session("ApplicationParameterView.strSort") = value
        End Set
    End Property
    Public Property indexStart() As String
        Get
            Return Session("ApplicationParameterView.indexStart")
        End Get
        Set(ByVal value As String)
            Session("ApplicationParameterView.indexStart") = value
        End Set
    End Property
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Try
                ...
                objFormModuleView.AddField("PK_SystemParameter_ID", "ID", 1, True, True, NawaBLL.Common.MFieldType.IDENTITY)
                objFormModuleView.AddField("SystemParameterGroup", "Parameter Group", 2, False, True, NawaBLL.Common.MFieldType.ReferenceTable, "SystemParameterGroup", "PK_SystemParameterGroup_ID", "SystemParameterGroup", "")
                objFormModuleView.AddField("SettingName", "Setting Name", 3, False, True, NawaBLL.Common.MFieldType.VARCHARValue)
                objFormModuleView.AddField("SettingValue", "Value", 4, False, True, NawaBLL.Common.MFieldType.VARCHARValue)
    ...
    Protected Sub Store_ReadData(sender As Object, e As StoreReadDataEventArgs)
        Try
            ...
            'strfilter = strfilter.Replace("FK_SystemParameterGroup_ID", "spg.SystemParameterGroup")
            ...
            Dim DataPaging As Data.DataTable = NawaDAL.SQLHelper.ExecuteTabelPaging("SystemParameter sp INNER JOIN SystemParameterGroup spg ON  sp.FK_SystemParameterGroup_ID = spg.PK_SystemParameterGroup_ID", "sp.PK_SystemParameter_ID,spg.SystemParameterGroup,sp.SettingName,(CASE WHEN sp.Hide = 1 THEN '..........' ELSE (CASE WHEN sp.fk_MFieldType_ID=11 THEN (SELECT aa.ApplicationAuthentication FROM SystemParameterpicklist aa WHERE aa.Pk_ApplicationAuthentication_ID=sp.SettingValue AND aa.SettingName=sp.SettingName) ELSE sp.SettingValue END ) END ) AS SettingValue,sp.[Active]", strfilter, strsort, intStart, intLimit, inttotalRecord)
            ''-- start paging ------------------------------------------------------------
        ...
        Try
            If Not cboExportExcel.SelectedItem Is Nothing Then
                If cboExportExcel.SelectedItem.Value = "Excel" Then
                    ...
                    Using objtbl As Data.DataTable = NawaDAL.SQLHelper.ExecuteTabelPaging("SystemParameter sp INNER JOIN SystemParameterGroup spg ON  sp.FK_SystemParameterGroup_ID = spg.PK_SystemParameterGroup_ID", "sp.PK_SystemParameter_ID,spg.SystemParameterGroup ,sp.SettingName,(CASE WHEN sp.Hide = 1 THEN '..........' ELSE (CASE WHEN sp.fk_MFieldType_ID=11 THEN (SELECT aa.ApplicationAuthentication FROM SystemParameterpicklist aa WHERE aa.Pk_ApplicationAuthentication_ID=sp.SettingValue AND aa.SettingName=sp.SettingName) ELSE sp.SettingValue END ) END ) AS SettingValue,sp.[Active]", Me.strWhereClause, Me.strOrder, 0, Integer.MaxValue, 0)
        ...
        Try
            If Not cboExportExcel.SelectedItem Is Nothing Then
                If cboExportExcel.SelectedItem.Value = "Excel" Then
                    ...
                    Using objtbl As Data.DataTable = NawaDAL.SQLHelper.ExecuteTabelPaging("SystemParameter sp INNER JOIN SystemParameterGroup spg ON  sp.FK_SystemParameterGroup_ID = spg.PK_SystemParameterGroup_ID", "sp.PK_SystemParameter_ID,spg.SystemParameterGroup  ,sp.SettingName,(CASE WHEN sp.Hide = 1 THEN '..........' ELSE (CASE WHEN sp.fk_MFieldType_ID=11 THEN (SELECT aa.ApplicationAuthentication FROM SystemParameterpicklist aa WHERE aa.Pk_ApplicationAuthentication_ID=sp.SettingValue AND aa.SettingName=sp.SettingName) ELSE sp.SettingValue END ) END ) AS SettingValue,sp.[Active]", Me.strWhereClause, Me.strOrder, Me.indexStart, NawaBLL.SystemParameterBLL.GetPageSize, 0)
