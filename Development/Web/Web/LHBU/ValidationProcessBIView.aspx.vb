...
Partial Class ValidationProcessBIView
    Inherits Parent
...
#Region "Session"
    ...
    Public Property ObjSettingPersonal As SettingPersonal
        Get
            Return Session("ValidationProcessViewBI.ObjSettingPersonal")
        End Get
        Set(ByVal value As SettingPersonal)
            Session("ValidationProcessViewBI.ObjSettingPersonal") = value
        End Set
    End Property

    Public Property ListFormInfo As List(Of FormInformation)
        Get
            Return Session("ValidationProcessViewBI.ListFormInfo")
        End Get
        Set(ByVal value As List(Of FormInformation))
            Session("ValidationProcessViewBI.ListFormInfo") = value
        End Set
    End Property

    Public Property ListSelectedForm As List(Of FormInformation)
        Get
            Return Session("ValidationProcessViewBI.ListSelectedForm")
        End Get
        Set(ByVal value As List(Of FormInformation))
            Session("ValidationProcessViewBI.ListSelectedForm") = value
        End Set
    End Property
    ...
    Private Function GetFormToValidate() As List(Of FormInformation)
        ...
        Try
            ...
            queryString = "SELECT fi.Kode KodeForm, fi.Nama NamaForm, fi.FK_Module_ID, " & vbCrLf &
                    "fi.FK_KelompokInformasi, ki.Label KelompokInfoStr, fi.Periode, prd.Label PeriodeStr,fi.VersionCode  " & vbCrLf &
                    "FROM dbo.ORS_FormInfo fi " & vbCrLf &
                    "LEFT JOIN dbo.Ref_KelompokInformasi ki " & vbCrLf &
                    "    ON ki.Sandi_Referensi = fi.FK_KelompokInformasi" & vbCrLf &
                    "LEFT JOIN dbo.Ref_Periode prd " & vbCrLf &
                    "    ON prd.Sandi_Referensi = fi.Periode" & vbCrLf &
                    "WHERE ISNULL(fi.Periode, '') <> '' AND ISNULL(fi.FK_KelompokInformasi, 0) <> 0 " & vbCrLf &
                ...
                If reader.HasRows = True Then
                    While reader.Read()
                        ...
                        For idx As Integer = 0 To reader.FieldCount - 1
                            ...
                            ElseIf (reader.GetName(idx) = "VersionCode") Then
                                objForm.VersionCode = If(IsDBNull(reader("VersionCode")), "", reader("VersionCode"))
                            End If
                        Next
...
#Region "Direct Events"
    ...
    Protected Sub TbFilter_OnChanged()
        If Not String.IsNullOrEmpty(2) Then
    ...
    Function CreateTableResult() As System.Data.DataTable
        Dim objdt As New Data.DataTable
        objdt.Columns.Add(New Data.DataColumn("forminfocode", GetType(String)))
        objdt.Columns.Add(New Data.DataColumn("KodeBank", GetType(String)))
        objdt.Columns.Add(New Data.DataColumn("KodeCabangBI", GetType(String)))
        objdt.Columns.Add(New Data.DataColumn("PeriodData", GetType(String)))
        objdt.Columns.Add(New Data.DataColumn("PeriodPelaporan", GetType(String)))
        objdt.Columns.Add(New Data.DataColumn("VersionCode", GetType(String)))
    ...
    Protected Sub BtnGenerate_OnClicked()
        Try
            If String.IsNullOrEmpty(2) Then
            ...
            'For Each itemdata As FormInformation In ListFormInfo
            '    strKode += itemdata.KodeForm & ","
            'Next
            'If strKode.Length > 0 Then strKode = strKode.Substring(0, Len(strKode) - 1)

            Dim dtresult As Data.DataTable = CreateTableResult()
            Dim oparambank As NawaDAL.SystemParameter = NawaBLL.SystemParameterBLL.GetSystemParameterByPk(9008)
            Dim strkodebank As String = ""
            If Not oparambank Is Nothing Then
                strkodebank = oparambank.SettingValue
            End If

            For Each itemdata As FormInformation In ListSelectedForm
                Dim onewrow As DataRow = dtresult.NewRow
                onewrow("forminfocode") = itemdata.KodeForm
                onewrow("KodeBank") = strkodebank
                onewrow("KodeCabangBI") = NawaDevBLL.SLIKParameterBLL.SessionSettingSLIKPersonal.KodeCabang
                onewrow("PeriodData") = NawaDevBLL.SLIKParameterBLL.SessionSettingSLIKPersonal.ReportDate.ToString("yyyy-MM-dd")
                onewrow("PeriodPelaporan") = itemdata.Periode
                onewrow("VersionCode") = itemdata.VersionCode
                dtresult.Rows.Add(onewrow)
            Next

            NawaDevBLL.ValidationReportBLL.ValidateBIInfo(dtresult)


            Dim objmodulelog As NawaDAL.Module = NawaBLL.ModuleBLL.GetModuleByModuleName("vw_EODLog")
            If Not objmodulelog Is Nothing Then
                Dim strModule As String = objmodulelog.PK_Module_ID
                strModule = NawaBLL.Common.EncryptQueryString(strModule, NawaBLL.SystemParameterBLL.GetEncriptionKey)
                Ext.Net.X.Redirect(String.Format(NawaBLL.Common.GetApplicationPath & objmodulelog.UrlView & "?ModuleID={0}", strModule), "Loading...")
...
#Region "Custom Variable [Untuk tampung data Form]"
    ...
    Public Class FormInformation
        ...
        Private versionCodeValue As String
