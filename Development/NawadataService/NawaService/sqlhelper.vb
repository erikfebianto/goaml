...
Public Class SQLHelper
    Public Shared ReadOnly strConnectionString As String = System.Configuration.ConfigurationManager.ConnectionStrings("NawaDataSql").ConnectionString
    ...
    Public Shared Function PrepareCommand(ByRef cmd As Data.Common.DbCommand, ByRef conn As Data.Common.DbConnection, ByRef cmdType As CommandType, ByRef cmdText As String, ByRef cmdParms As SqlParameter()) As Boolean
        ...
        End Try
    End Function
    Public Shared Function ExecuteTable(ByVal oconn As Data.Common.DbConnection, otrans As Entity.DbContextTransaction, ByVal cmdType As CommandType, ByVal cmdText As String, Optional ByVal cmdParms As SqlParameter() = Nothing) As DataTable

        Dim cmd As Data.Common.DbCommand = oconn.CreateCommand
        ' Dim conn As SqlConnection = New SqlConnection(connString)
        ' Dim oDataAdapter As New SqlDataAdapter
        Dim oDataTable As New DataTable
        Try
            PrepareCommand(cmd, oconn, cmdType, cmdText, cmdParms)
            'cmd.Parameters = cmdParms
            cmd.Transaction = otrans.UnderlyingTransaction
            Using objreader As Data.Common.DbDataReader = cmd.ExecuteReader()
                oDataTable.Load(objreader)
            End Using
    ...
    Public Shared Function GetSQLCommandTimeOut() As Integer
        '        Return CInt(ExecuteScalar(strConnectionString, CommandType.Text, "SELECT sp.SettingValue FROM SystemParameter sp WHERE sp.PK_SystemParameter_ID=5", Nothing))

        Return CInt(System.Configuration.ConfigurationManager.AppSettings("CommandTimeout"))

    End Function

   
    Public Shared Function ExecuteTabelPaging(ByVal Tables As String, ByVal Field As String, ByVal strWhereClause As String, ByVal Sort As String, ByVal PageIndex As Int64, ByVal PageSize As Int64, ByRef intcount As Integer) As DataTable
        ...
        Try
            ...
            SqlCmd.CommandTimeout = GetSQLCommandTimeOut()
            SqlCmd.CommandType = CommandType.StoredProcedure
            SqlCmd.CommandText = "usp_Pagingdata2012"
            ...
            If reader.NextResult Then
                reader.Read()
                intcount = CType(reader(0), Integer)
            End If
    ...
    Public Shared Function ExecuteXmlReader(ByVal connString As String, ByVal cmdType As CommandType, ByVal cmdText As String, ByVal cmdParms As SqlParameter()) As Object
        Dim cmd As SqlCommand = New SqlCommand
        Dim conn As SqlConnection = New SqlConnection(connString)
        Try
            PrepareCommand(cmd, conn, cmdType, cmdText, cmdParms)
    ...
    Public Shared Function ExecuteScalar(ByVal oconn As Data.Common.DbConnection, otrans As Entity.DbContextTransaction, ByVal cmdType As CommandType, ByVal cmdText As String, ByVal cmdParms As SqlParameter()) As Object
        Dim cmd As Data.Common.DbCommand = oconn.CreateCommand
        ...
        '    PrepareCommand(cmd, conn, cmdType, cmdText, cmdParms)
        '    Dim val As Object = cmd.ExecuteScalar()
        '    cmd.Transaction = CType(otrans.UnderlyingTransaction, SqlTransaction)
        '    cmd.Parameters.Clear()
        '    Return val
        'Catch ex As SqlException
        '    Throw
