...
Public Class EMail
    ...
    Private m_HeaderList As Dictionary(Of String, String)
    Private mylog As New NawaConsoleLog
    Private m_MailAttachmentFromStream As Dictionary(Of String, Stream)
    ...
    Public Property HeaderList() As Dictionary(Of String, String)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property MailAttachmentFromStream() As Dictionary(Of String, Stream)
        Get
            MailAttachmentFromStream = m_MailAttachmentFromStream
        End Get
        Set(ByVal Value As Dictionary(Of String, Stream))
        ...
        Try
            ...
            If isValidMail() Then
                ...
                If m_HeaderList.Count > 0 Then
                    For Each item As KeyValuePair(Of String, String) In m_HeaderList
                        email.Headers.Item(item.Key) = item.Value
                    Next
                ...
                Try
                    ...
                    Using objDb As NawaDataEntities = New NawaDataEntities
                        ...
                        If Not objsysparam Is Nothing Then
                            strpassword = DecryptRijndael(objsysparam.SettingValue, objsysparam.EncriptionKey)
                        End If
                    End Using
    ...
    ''' <returns>Return TRUE if send mail done successfully, otherwise return FALSE</returns>
    ''' <remarks></remarks>
    Public Function SendEmail(DicAttachment As Dictionary(Of String, Stream)) As Boolean
        'For a Windows Forms Application, add a reference to 
        'System.Web.dll using [Project Add References]
        ...
        Try
            ...
            If isValidMail() Then
                ...
                If Not DicAttachment Is Nothing Then
                    For IntCounter As Integer = 0 To DicAttachment.Count - 1
                        att = New Attachment(DicAttachment.Item(DicAttachment.Keys(IntCounter)), DicAttachment.Keys(IntCounter).ToString())
                        email.Attachments.Add(att)
                        att = Nothing
                    Next
                End If
